...
export default function BlockListShared(props: Props) {
  ...
  function handleRescind(memberClaim) {
    doOpenModal(MODALS.CONFIRM, {
      title: __('Rescind'),
      body: (
        <>
          <p>{__('Rescind the invitation for the following member?')}</p>
  ...
  function getRowElem(label: string, value: any) {
    return (
      <tr>
        <td>{label}</td>
        <td>{value}</td>
  ...
  function getSblStatusElem(isActive) {
    // `isActive === undefined` means "my SBL". I think the only way to check if it's being used is to check whether we
    // accepted any invitations.
    if (
      isActive ||
      (isActive === undefined && (!sblInvited || !sblInvited.some((invite) => invite.invitation.status === 'accepted')))
    ) {
      return (
        ...
        <ClaimList
          ...
          renderProperties={(claim) => {
            ...
            if (member && member.status === 'expired') {
              return <div className="sbl_invite_expired">{__('Expired')}</div>;
            }
            return null;
          }}
          renderActions={(claim) => {
                    ...
                    <div className="section">
                      <table className="table table--condensed table--publish-preview table--no-row-lines">
                        <tbody>
                          ...
                          {getRowElem(__('Message'), i.invitation.message)}
                        </tbody>
                      </table>
  ...
  return (
    <div className={!fetchedOnce ? 'card--disabled' : isUpdating ? 'card--disable-interaction' : ''}>
      ...
      <div className="section">{getMySblElem()}</div>
      <div className="section">{getInvitedSblElem()}</div>
    </div>
  );
}
