...
function handlePunctuation(value) {
  const idSeparatorIndex =
    (value.indexOf(':') >= 0 && value.indexOf(':')) || (value.indexOf('#') >= 0 && value.indexOf('#'));

  let punctuationIndex;
  punctuationMarks.some((p) => {
    if (idSeparatorIndex) {
      punctuationIndex = value.indexOf(p, idSeparatorIndex + 1) >= 0 && value.indexOf(p, idSeparatorIndex + 1);
    } else {
      punctuationIndex = value.lastIndexOf(p) >= 0 && value.lastIndexOf(p);
    }
